...
{
    //On the code bellow we created a temporary array
    //with the size of the previous current_size + 1
    //we added 1 because we needed to create an empty space
    //in our memory to add a new team
  Team* tempArr = new Team[current_size+1];
  ...
  current_size++;
    ...
    // On the code bellow we used current_size - 2 in order
    // to get the index of the previous team and we added 1 
    // to the result of getId to generate a new ID for the team
    // that the user will add.
    newTeam->setId(tarr[current_size-2].getId()+1);
    tarr[current_size-1] = *newTeam;
